...
public class ServerCreationService : BackgroundService
{
        ...
        if (request.core.Contains("forge") && request.core.EndsWith(".jar") && !request.core.Contains("arclight"))
        {
            ...
            try
            {
                ...
                using (var scope = _scopeFactory.CreateScope())
                {
                    ...
                    // 拼接java地址
                    string javaPath = request.java.Contains("MSLX://Java/")
                        ? Path.Combine(ConfigServices.GetAppDataPath(), "DaemonData", "Tools", "Java",
                    ...
                    // 修改服务器配置的core参数
                    string launchArgs = ExtractNeoForgeArgsPath(Path.Combine(server.Base, PlatFormServices.GetOs() == "Windows" ? "run.bat" : "run.sh")) ?? "";
                    if (string.IsNullOrEmpty(launchArgs))
                    {
                        throw new Exception("未找到启动文件！");
                    }
                    McServerInfo.ServerInfo serverNew = new McServerInfo.ServerInfo
                    {
                        ID = serverId,
                        Name = request.name,
                        Base = request.path ??
                    ...
                    ConfigServices.ServerList.UpdateServer(serverNew);
    ...
    public static string? ExtractNeoForgeArgsPath(string batFilePath)
    {
        ...
        try
        {
            ...
            string pattern = @"java\s+@user_jvm_args\.txt\s+(@\S+)";
            Match match = Regex.Match(batFileContent, pattern);
