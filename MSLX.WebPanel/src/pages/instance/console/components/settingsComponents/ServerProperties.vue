...
<template>
  <div class="flex flex-col mx-auto w-full relative pb-8">
    <div
    ...
    >
      ...
      <t-space size="small" class="w-full sm:w-auto justify-end">
        <t-button
          variant="outline"
          class="!rounded-lg !bg-zinc-50 dark:!bg-zinc-800/50 !border-zinc-200 dark:!border-zinc-700 hover:!bg-zinc-100 dark:hover:!bg-zinc-800 !text-zinc-600 dark:!text-zinc-300 transition-colors"
          :loading="loading"
          @click="loadData"
        >
    ...
    <t-loading :loading="loading" text="正在读取配置文件...">
      ...
      >
        <div
          v-if="!loading && renderList.length === 0"
          class="py-16 flex items-center justify-center text-sm font-medium text-[var(--td-text-color-secondary)]"
        >
          无法找到配置项或文件为空
        </div>
          ...
          >
            <div class="flex-1 md:max-w-[40%] pr-0 md:pr-8 mb-3 md:mb-0">
              <div
                ...
                :class="item.isUnknown ? 'text-amber-600 dark:text-amber-500' : 'text-[var(--td-text-color-primary)]'"
              >
                {{ item.label }}
              </div>
              ...
              <div class="flex items-center mb-1.5" v-if="!item.isUnknown">
                <span
                  class="font-mono text-[11px] text-[var(--td-text-color-secondary)] bg-zinc-100 dark:bg-zinc-800/80 px-1.5 py-0.5 rounded tracking-wider shadow-inner"
                  >{{ item.key }}</span
                >
              </div>
    ...
    <div
      class="sticky bottom-1 z-50 ml-auto w-max flex items-center gap-2 p-1.5 mt-4 mb-2 border border-zinc-200/80 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-xl shadow-black/10 dark:shadow-black/40 rounded-full transition-all hover:shadow-lg"
    >
      <t-button
        theme="default"
        variant="text"
        class="!rounded-full !px-5 text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 transition-colors"
        @click="loadData"
        :disabled="loading"
      >
        重置更改
