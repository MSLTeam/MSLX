<template>
  <div
    class="global-layout-bg"
    :class="{ 'custom-theme-enabled': enableCustomTheme }"
...
const themeConfig = ref({
  lightBgUrl: defaultLightBg,
  darkBgUrl: defaultDarkBg,
  lightBgOpacity: 1,     // 白天背景图透明度 (0-1)
  darkBgOpacity: 1,      // 黑夜背景图透明度 (0-1)
  lightCompOpacity: 0.4, // 白天组件透明度
  darkCompOpacity: 0.6   // 黑夜组件透明度
});

const customThemeVars = computed(() => {
  const c = themeConfig.value;
  return {
    '--bg-img-light': `url('${c.lightBgUrl}')`,
    '--bg-img-dark': `url('${c.darkBgUrl}')`,
    '--bg-op-light': c.lightBgOpacity,
    '--bg-op-dark': c.darkBgOpacity,
    '--comp-op-light': c.lightCompOpacity,
    '--comp-op-dark': c.darkCompOpacity,
  };
});
...
.global-layout-bg.custom-theme-enabled {
  ...
  // 布局透明
  :deep(.t-layout), :deep(.t-layout__content), :deep(.t-content) {
    background: transparent !important;
  }

  // 清除内部背景
  :deep(.t-menu), :deep(.t-head-menu), :deep(.t-default-menu),
  :deep(.t-default-menu__inner), :deep(.t-head-menu__inner), :deep(.t-menu__scroll),
  :deep(.mslx-webpanel-side-nav), :deep(.mslx-webpanel-header-layout),
  :deep(.t-card__header), :deep(.t-card__footer), :deep(.t-card__body) {
    background: transparent !important;
  ...
  // 白天组件样式
  :deep(.t-layout__sider), :deep(.t-aside), :deep(.t-layout__header),
  :deep(.t-header), :deep(.t-card), :deep(.t-input) {
  ...
  :deep(.t-input) { border-color: rgba(255, 255, 255, 0.5) !important; }

  // 黑夜组件样式
  &.dark, :global(html[theme-mode="dark"]) & {
    :deep(.t-layout__sider), :deep(.t-aside), :deep(.t-layout__header),
    :deep(.t-header), :deep(.t-card) {
